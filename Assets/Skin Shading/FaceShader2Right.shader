Shader "FaceShaderSimpleRight" {
	...
	Properties {
		...
		_SmoothNose("Smooth Nose",Float) = 1
		_SmoothNoseSize("Smooth Nose Size",Range(0,1)) = 0.1
	}
	...
	SubShader {
		...
		#pragma surface surf Skin fullforwardshadows vertex:vert
		...
		float3 RotateAroundInDegrees(float3 vertex, float degrees)
		{
		...
		float3 ApplyNormalTransition(float3 normal, float2 uv)
		{
			if (uv.x < _SmoothNoseSize)
			{
				float3 rightNormal = RotateAroundInDegrees(normal, 180);
				rightNormal.y *= -1.0f;

				return lerp(rightNormal, normal, clamp(uv.x / _SmoothNoseSize, 0.5, 1));
			}
		...
		void surf(Input IN, inout SurfaceOutputSkin o) {
			...
			if (_SmoothNose)
			{
				//this is to improve nose lightning
				o.Normal = ApplyNormalTransition(o.Normal, uv);
			}
...
//			fixed3 blurredWorldNormal = UnpackNormal( tex3Dlod ( _Tex3dBump, float4 ( uv, _Zamount, _BumpBias ) ) );
			...
			{
				//this is to improve nose lightning
				blurredWorldNormal = ApplyNormalTransition(blurredWorldNormal, uv);
			}
			
			if (_invertNormals == 1)
				blurredWorldNormal.x *= -1;
...
//			o.Albedo = _Color*0.5;
		...
		inline fixed4 CookTorrenceLight (SurfaceOutputSkin s, half3 viewDir, UnityGI gi) {
			...
			float dotNLBlur = dot( normalize(s.NormalBlur), normalize(gi.light.dir));
			...
			brdfUV.x = clamp(dotNLBlur * 0.5 + 0.5,0.1,0.9); //don't really why but it works better if we clamp between [0.1:0.9], otherwise shadow/light interference appears
